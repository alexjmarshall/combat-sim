...
async function newGame() {
  ...
  try {
    state = await api("/api/new-game", { method: "POST" });
...
function renderAtkCommit(panel, p) {
  ...
  $("#btn-commit").addEventListener("click", async () => {
    await postAction("/api/atk-commit", { n: parseInt(slider.value, 10) });
  });
  $("#btn-end-turn").addEventListener("click", async () => {
    await postAction("/api/atk-commit", { end_turn: true });
  });
}
...
function renderAtkManeuver(panel, p) {
  ...
  $$("#action-panel .maneuver-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      await postAction("/api/atk-maneuver", body);
    });
  });
}
...
function renderDefManeuver(panel, p) {
  ...
  $$("#action-panel .maneuver-btn").forEach((btn) => {
    btn.addEventListener("click", async () => {
      ...
      await postAction("/api/def-maneuver", body);
    });
  });
}
